...
class FileTree {
  ...
  async importDump (username, buffer) {
    await this._lock(username);
    try {
      const unzipper = require('unzipper');
      const directory = await unzipper.Open.buffer(buffer);
      await directory.extract({ path: this._resolvePath(username, 'storage'), concurrency: 5 });
  ...
  async dump (username, response) {
    ...
    archive.directory(this._resolvePath(username, 'storage'), false);
    archive.finalize();
    archive.on('end', () => {
      console.log('ended');
