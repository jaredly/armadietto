...
class WebFinger extends Controller {
  ...
  isHTTPS () {
    ...
    // console.log(this.request.secure, this.request.connection && this.request.connection.authorized, this.server._forceSSL, this.request.socket && this.request.socket.secure);
    return (
      this.request.secure ||
      this.server._forceSSL ||
      (this.request.connection && this.request.connection.authorized !== undefined) ||
      (this.request.socket && this.request.socket.secure) ||
      (this.request.headers['x-forwarded-ssl'] === 'on') ||
      this.request.headers['x-forwarded-scheme'].match(/https/i) ||
      this.request.headers['x-forwarded-proto'].match(/https/i));
  }
