...
class Armadietto {
  ...
  handle (req, res) {
    ...
    if (req.headers['content-type'] && req.headers['content-type'].startsWith('multipart/form-data')) {
      ...
      busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
        ...
        file.on('end', () => {
          files[fieldname] = { filename, encoding, mimetype, buf: Buffer.concat(bufs) };
        });
      });
  ...
  async dispatch (req, res) {
    ...
    if (uri.pathname === 'dump') {
      const users = new Users(this, req, res);
      if (method === 'GET') return users.showDump();
      if (method === 'POST') return users.doDump(this._store);
    }
